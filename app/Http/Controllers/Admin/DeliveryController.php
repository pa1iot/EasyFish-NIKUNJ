...
class DeliveryController extends Controller
{
    ...
    public function update_profile(Request $request)
    {
        ...
        $rules = array(
            'username' => ['required', 'regex:/^[\w-]*$/', 'max:255', Rule::unique('users')->ignore($token, 'id')->where(function ($sql) {
                $sql->where('drop_status', '=', 'no');
            })],
            'email' => ['required', 'email', 'max:255', Rule::unique('users')->ignore($token, 'id')->where(function ($sql) {
                $sql->where('drop_status', '=', 'no');
            })],
    ...
    {
        $userData['data'] = Members::getdeliveryboyData();
        return view('admin.delivery.administrator', ['userData' => $userData]);
    }

    public function add_administrator()
    {

        return view('admin.delivery.add-administrator');
    }
    ...
    public function save_administrator(Request $request)
    {
        ...
        $rules = array(
            'username' => ['required', 'regex:/^[\w-]*$/', 'max:255', Rule::unique('users')->where(function ($sql) {
                $sql->where('drop_status', '=', 'no');
            })],
    ...
    public function edit_administrator($token)
    {
        ...
        $edit['userdata'] = Members::editData($token);
        return view('admin.delivery.edit-administrator', ['edit' => $edit, 'token' => $token]);
    }
    ...
    public function update_administrator(Request $request)
    {
        ...
        $rules = array(
            'username' => ['required', 'regex:/^[\w-]*$/', 'max:255', Rule::unique('users')->ignore($token, 'user_token')->where(function ($sql) {
                $sql->where('drop_status', '=', 'no');
            })],
            'email' => ['required', 'email', 'max:255', Rule::unique('users')->ignore($token, 'user_token')->where(function ($sql) {
                $sql->where('drop_status', '=', 'no');
            })],
