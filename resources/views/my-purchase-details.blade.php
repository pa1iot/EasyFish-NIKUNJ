...
<section class="headerbg"
    ...
    <div class="container text-left">
        ...
        <p class="mb-0"><a href="{{ URL::to('/') }}">{{ Helper::translation(1913,$translate) }}</a> <span class="split">&gt;</span>
            <span>{{ Helper::translation(2110,$translate) }}</span></p>
    </div>
</section>
<main role="main">
    <div class="container mt-3">
        <div class="row">
            <div class="col-md-12" align="right">
                <a href="{{ URL::to('/my-purchase') }}"
                   class="btn button-color">&lt; {{ Helper::translation(2088,$translate) }}</a><a
                    href="{{ url('/invoice') }}/{{ $purchase->purchase_token }}" class="btn btn-danger ml-1"><i
            ...
            <div class="col-md-12">
                ...
                <div class="form-group row">
                    <div class="col-sm-3">
                        ...
                        @if($purchase->payment_status == 'completed') <span
                            class="badge badge-success">{{ Helper::translation(2084,$translate) }}</span> @else <span
                            class="badge badge-danger">{{ Helper::translation(2085,$translate) }}</span> @endif
                    </div>
                </div>
                    ...
                    <table id="bootstrap-data-table-export" class="table table-striped table-bordered">
                        ...
                        @foreach($product['view'] as $product)
                            <tr>
                                    ...
                                    <a href="{{ url('/product') }}/{{ $product->product_slug }}">
                                        @if($product->product_image != '')
                                            <img
                                                ...
                                                class="img-thumb" alt="{{ $product->product_name }}"/>@else <img
                                            src="{{ url('/') }}/public/img/no-image.jpg" class="img-thumb"
                                            alt="{{ $product->product_name }}"/>  @endif
                                    </a>
                                </td>
                                <td>{{ $product->quantity }}
                                    X {{ $allsettings->site_currency_symbol }}{{ $product->price }}</td>
                                    ...
                                    @if($product->order_status == 'completed')
                                        @if($product->payment_status != $payment_status_buyer or $product->payment_status == $payment_status_vendor)
                                            <a href="javascript:void(0);" class="badge badge-success"
                                               data-toggle="modal" data-target="#sem-rate-{{ $product->ord_id }}"><i
                                                    class="fa fa-star"></i> {{ Helper::translation(2114,$translate) }}
                                            </a>                            @else
                                            <span>---</span>
                                        @endif
                                        @if($product->product_type == 'digital')
                                            @if($product->payment_status != $payment_status_buyer or $product->payment_status == $payment_status_vendor)
                                                @if($allsettings->site_s3_storage == 1)
                                                    <br/><a href="{{ $fileurl }}" class="badge badge-primary"
                                                            download><i
                                                            class="fa fa-download"></i> {{ Helper::translation(2116,$translate) }}
                                                    </a>
                                                @else
                                                    <br/><a
                                                        href="{{ url('/') }}/public/storage/product/{{ $product->product_file }}"
                                    ...
                                    @if($product->product_type == 'physical')
                                        @if($product->payment_status != $payment_status_buyer)
                                            <a href="javascript:void(0);" class="btn btn-danger btn-sm"
                                               data-toggle="modal"
                                               data-target="#sem-track-{{ $product->ord_id }}">{{ Helper::translation(2098,$translate) }}</a>
                                        @else
                                            <span>---</span>
                                        @endif
                                    @else
                                ...
                                <td>
                                    @if($product->payment_status != $payment_status_buyer or $product->payment_status == $payment_status_vendor)
                                        <a href="{{ url('/conversation-to-vendor') }}/{{ $product->username }}/{{ $encrypter->encrypt($product->ord_id) }}"
                                           class="btn btn-primary btn-sm">{{ Helper::translation(2915,$translate) }}</a>
                                    @else
                                        <span>---</span>
                                    @endif
                                </td>
                                ...
                                <div class="modal-dialog modal-lg modal-dialog-centered">
                                    <div class="modal-content">
                                        ...
                                        <div class="modal-body">
                                            <div class="card-body">
                                                <h6>{{ Helper::translation(2076,$translate) }}
                                                    : {{ $product->ord_id }}</h6>
                                                <article class="card">
                                                    <div class="card-body row">
                                                        <div class="col">
                                                            <strong>{{ Helper::translation(2094,$translate) }}:</strong>
                                                            <br>{{ $delivery_date }}</div>
                                                        <div class="col">
                                                            <strong>{{ Helper::translation(1915,$translate) }}:</strong>
                                                            <br> Order has been {{ $product->order_tracking }}</div>
                                                        <div class="col">
                                                            <strong>{{ Helper::translation(2117,$translate) }}:</strong>
                                                            <br> {{ $product->ord_id }} </div>
                                                    </div>
                                                </article>
                                                <div class="track">
                                                    @if($product->order_tracking == $track_placed)
                                                        <div class="step active"><span class="icon"> <i
                                                                ...
                                                                class="text">{{ Helper::translation(2118,$translate) }}</span>
                                                        </div>
                                                        <div class="step"><span class="icon"> <i
                                                                    class="fa fa-square"></i> </span> <span
                                                                class="text">{{ Helper::translation(2119,$translate) }}</span>
                                                        </div>
                                                        <div class="step"><span class="icon"> <i
                                                                    class="fa fa-truck"></i> </span> <span
                                                                class="text">{{ Helper::translation(2120,$translate) }}</span>
                                                        </div>
                                                        <div class="step"><span class="icon"> <i
                                                                    class="fa fa-handshake-o"></i> </span> <span
                                                                class="text">{{ Helper::translation(2121,$translate) }}</span>
                                                        </div>
                                                    @endif
                                            ...
                                            <div class="row">
                                                <div class="col-md-6">
                                                    @if($product->product_image != '')
                                                        <img
                                                            src="{{ url('/') }}/public/storage/product/{{ $product->product_image }}"
                                                            class="refund-img" alt="{{ $product->product_name }}"/>@else
                                                ...
                                                <div class="col-md-6">
                                                    <form action="{{ route('rating') }}" class="setting_form"
                                                          method="post" enctype="multipart/form-data">
                                                        {{ csrf_field() }}
                                                        <div class="form-group">
                                                            <label
                                                                for="email">{{ Helper::translation(2122,$translate) }}</label><br/>
                                                            <div>
                                                        ...
                                                        <div class="form-group">
                                                            <label
                                                                for="pwd">{{ Helper::translation(2123,$translate) }}</label>
                                                        ...
                                                        <input type="hidden" name="order_id"
                                                               value="{{ $product->ord_id }}">
                                                        <button type="submit"
                                                                class="btn button-color">{{ Helper::translation(1919,$translate) }}</button>
                                                    </form>
                                                </div>
                                            </div>
                                        </div>
                                    ...
                                    <div class="modal-content">
                                        ...
                                        <div class="modal-body">
                                            <div class="row">
                                                ...
                                                <div class="col-md-6">
                                                    <form action="{{ route('refund-request') }}" class="setting_form"
                                                          method="post" enctype="multipart/form-data">
                                                        {{ csrf_field() }}
                                                        <input type="hidden" name="product_id"
                                                               value="{{ $product->product_id }}">
                                                        ...
                                                        <div class="form-group">
                                                            <label
                                                                for="email">{{ Helper::translation(2125,$translate) }}</label>
                                                            <input type="text" class="form-control" name="reason"
                                                                   required="required">
                                                        </div>
                                                        <div class="form-group">
                                                            <label
                                                                for="pwd">{{ Helper::translation(2126,$translate) }}</label>
